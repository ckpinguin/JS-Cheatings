...
## `this` Binding
Wie die Closures ist auch `this` ein Teil des _Execution Context_. Es wird auch als _context object_ bezeichnet, weil es definiert in welchem Kontext die Ausführung (execution) aktiviert wurde (im Gegensatz zur Closure, die statisch/lexikalisch zum Deklarationszeitpunkt gesichert wird). Merke: `this` ist keine Referenz auf die Funktion selbst (wie ein gedachtes self o.ä.) und ebensowenig eine Referenz auf den lexikalen Scope. `this` ist ein Binding, welches wie erwähnt zum Zeitpunkt des Funktionsaufrufes erstellt wird.
